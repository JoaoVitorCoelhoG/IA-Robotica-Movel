...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        for k in range(1,num_neighbors):
            neighbors_list.append(theta + delta*np.array([cos(2*k*pi/num_neighbors), sin(2*k*pi/num_neighbors)]))
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        """
        # Todo: Implement
        rotation = random.uniform(-pi,pi)
        random_neighbor = theta + delta*np.array([cos(rotation), sin(rotation)])
    ...
    def schedule(i):
        ...
        """
        # Todo: Implement
        temperature = temperature0/(1 + i**2)
        return temperature
