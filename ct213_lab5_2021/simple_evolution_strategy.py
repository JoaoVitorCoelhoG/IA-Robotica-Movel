...
class SimpleEvolutionStrategy:
    ...
    def tell(self, fitnesses):
        ...
        # The input is the fitness function
        # Fitnesses is one array of size of self.samples
        # each term is the value of each element from population_size
        # i neer order fitnsesses

        indices = np.argsort(fitnesses)
        best_samples = self.samples[indices[0:self.mu], :]
        #if fitnesses(self.m) > best_samples.mean(axis=0):
        self.m = best_samples.mean(axis=0)
